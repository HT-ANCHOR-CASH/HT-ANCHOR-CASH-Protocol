...
contract Treasury is ContractGuard, Epoch {
        ...
        checkOperator
    {
        ...
        if (boardroomReserve > 0) {
            uint256 boardroomTotal = boardroomReserve.mul(95).div(100);
            IERC20(cash).safeApprove(boardroom, boardroomTotal);
            IBoardroom(boardroom).allocateSeigniorage(boardroomTotal);
            emit BoardroomFunded(now, boardroomTotal);
        }
    }
